...
export function KioskLanding(){
  ...
  if(theme.showStartScreen!==false&&!started){return <section className="kiosk-start-screen"><div className="kiosk-start-media" style={{backgroundImage:`linear-gradient(180deg,rgba(18,12,8,.08),rgba(18,12,8,.62)),url(${theme.heroImageUrl||"/images/szolgaltatasok.jpg"})`}}><div className="kiosk-start-brand"><img src={theme.logoUrl||"/images/kleo_logo@2x.png"} alt="Kleopátra"/><span>{locationName}</span></div><div className="kiosk-start-copy"><span className="kiosk-start-kicker">ÖNKISZOLGÁLÓ KIOSK · GYÖNGYÖS</span><h1>{theme.startTitle||"Üdvözlünk a Kleopátra Szépségszalonban!"}</h1><p>{theme.startSubtitle||"Érintsd meg a képernyőt a választás megkezdéséhez."}</p></div><div className="kiosk-start-actions"><div className="kiosk-fixed-location"><span>HELYSZÍN</span><b>{locationName}</b></div><button className="kiosk-start-button" onClick={start}>{theme.startButtonText||"Kezdés"}<span>→</span></button></div></div></section>}
