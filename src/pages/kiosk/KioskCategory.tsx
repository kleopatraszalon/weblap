...
export function KioskCategory(){
 ...
 return <div className="kiosk-order-layout">
  <aside className="kiosk-category-rail"><button className="kiosk-back-home" onClick={()=>nav("/kiosk")}>← Főmenü</button><button className="kiosk-retail-rail-button" onClick={()=>nav("/kiosk/products")}>🛍 <span>Termékeladás</span><small>KÁVÉ · ITAL · CSOKI</small></button><div className="kiosk-rail-title">Menü</div><div className="kiosk-rail-list">{allCats.map((c)=><button key={`${c.type}-${c.id}`} className={catalogType===c.type&&activeCategory?.id===c.id?"active":""} onClick={()=>selectCategory(c,c.type)}><KioskSemanticArt kind={c.type} name={c.name} source={c.image_path}/><span>{c.name}</span><small>{c.type==="product"?"TERMÉK":"SZOLGÁLTATÁS"}</small></button>)}</div></aside>
